...
date = datetime.datetime(2011,04,20,0)
# open dataset.
dataset =\
...
im1 = m.pcolor(x,y,sst,shading='flat',cmap=pp.cm.jet)
im2 = m.pcolor(x,y,ice,shading='flat',cmap=pp.cm.gist_gray)
# draw parallels and meridians, but don't bother labelling them.
m.drawparallels(np.arange(-90.,120.,30.))
m.drawmeridians(np.arange(0.,420.,60.))
# add colorbar
#cb = m.colorbar(im1,"bottom", size="5%", pad="2%")
c = pp.colorbar(im1,orientation='horizontal')
# add a title.
ax.set_title('SST and ICE analysis for %s'%date)
pp.savefig('plotsst.png')
pp.show()
