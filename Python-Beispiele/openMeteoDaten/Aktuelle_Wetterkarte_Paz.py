...
for x,y,p in zip(xlows, ylows, lowvals):
    if x < m.xmax and x > m.xmin and y < m.ymax and y > m.ymin:
        ...
        if not dist or min(dist) > dmin:
            pp.text(x,y,'L',fontsize=14,fontweight='bold',
                    ha='center',va='center',color='b')
            pp.text(x,y-yoffset,repr(int(p)),fontsize=9,
...
for x,y,p in zip(xhighs, yhighs, highvals):
    if x < m.xmax and x > m.xmin and y < m.ymax and y > m.ymin:
        dist = [np.sqrt((x-x0)**2+(y-y0)**2) for x0,y0 in xyplotted]
        if not dist or min(dist) > dmin:
            pp.text(x,y,'H',fontsize=14,fontweight='bold',
                    ha='center',va='center',color='r')
            pp.text(x,y-yoffset,repr(int(p)),fontsize=9,
                    ha='center',va='top',color='r',
                    bbox = dict(boxstyle="square",ec='None',fc=(1,1,1,0.5)))
            xyplotted.append((x,y))
pp.title('Mean Sea-Level Pressure (with Highs and Lows) %s' % YYYYMMDDHH)
pp.show()
pp.savefig('plothighsandlows.png')
